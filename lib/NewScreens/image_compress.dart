...
Future<Uint8List> compressImage(
  ...
  if (imageBytes.length <= targetSizeBytes) {
    print(
        'Image already below 0.7MB (${(imageBytes.length / 1024 / 1024).toStringAsFixed(2)}MB), skipping compression');
    return imageBytes;
  }
...
Future<Uint8List> _compressWebImageToTargetSize(
  ...
  int attempts = 0;
  final int maxAttempts = 2; // SPEED PRIORITY - reduced from 8 to 2
  ...
  while (attempts < maxAttempts) {
    ...
    Uint8List compressed = await resizeWebImage(imageBytes, targetWidth,
